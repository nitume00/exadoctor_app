...
// Actions
import * as RecipeActions from '@reduxx/recipes/actions';
...
const mapStateToProps = state => ({
  user: state.user,
  favourites:
    state.recipe && state.recipe.favourites ? state.recipe.favourites : null,
});
...
class RecipeCard extends Component {
  ...
  isFavourite = () => {
    ...
    if (recipe && recipe.id && favourites) {
      if (favourites.length > 0 && favourites.indexOf(recipe.id) > -1)
        return true;
    }
